...
Дополнительно компонент извлекает выражения или код из таких узлов как **Калькулятор**, **Калькулятор (переменные)**, **JavaScript**, **Python**, если такие узлы были поданы на вход компонента.

Компонент участвует при парсинге узлов всего пакета в рамках работы компонента **[Мета-данные пакета](./package-metadata.md)**.
...
| Код узла             | ![](./img/string.svg) Строковый            |