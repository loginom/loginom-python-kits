...
Мета-компонент, который использует внутри компонент **simple.fitter** для перекодирования категориальных значений методом `LeaveOneOutEncoder` из библиотеки [Caterory Encoders](https://contrib.scikit-learn.org/category_encoders). 
...
| 10 | Целевое поле                | ![](./img/string.svg) Строковый           | target                  |
| 11 | Виртуальное окружение       | ![](./img/string.svg) Строковый           |null                     |
...
10. **Целевое поле** —  имя столбца в датасете с целевым значением.
11. **Виртуальное окружение** — путь к файлу интерпретатора Python.
